  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::mav_system_msgs::CpuInfo_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.cpu_percent.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.cpu_percent[i]);
    }
    if (v.cpu_percent.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "heaviest_processes: ";
    if (v.heaviest_processes.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.heaviest_processes.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::mav_system_msgs::ProcessInfo_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.heaviest_processes[i]);
    }
    if (v.heaviest_processes.empty() || false)
      s << "]";
  }
